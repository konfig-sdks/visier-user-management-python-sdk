# coding: utf-8

"""
    Visier User Management APIs

    Visier APIs for managing users within an organization

    The version of the OpenAPI document: 22222222.99201.1200
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel

from visier_user_management_python_sdk.pydantic.permission_response_dto import PermissionResponseDTO

class UserGroupGetAPIResponseDTO(BaseModel):
    # The unique identifier associated with the user group.
    user_group_id: typing.Optional[str] = Field(None, alias='userGroupId')

    # An identifiable user group name to display in Visier, such as \"Leadership User Group\".
    display_name: typing.Optional[str] = Field(None, alias='displayName')

    # A list of objects representing the user's permissions.
    permissions: typing.Optional[typing.List[PermissionResponseDTO]] = Field(None, alias='permissions')
    class Config:
        arbitrary_types_allowed = True
